...
from tp_final.src.volume_data_processor import create_volume_bars, create_dollar_bars, create_price_change_bars
...
logging.debug(df[['close', 'close_pct_change']].describe())
...
def analyze_returns_properties(returns: pd.Series, bar_type: str):
    """
    Performs and logging.debugs a statistical analysis of a returns series.
    """
...
# %%
df['close'].pct_change().abs().dropna().max() * 4 * 0.3
...
# %%
pd.to_datetime(df_1m['date'], format=)
