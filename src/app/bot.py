...
class Bot:
    def __init__(
        self, config: dict[str, Any], exchange_client: Any, strategy: Strategy
    ):
        ...
        self.capital_allocation: float = config["bot"]["capital_allocation"][
            "max_capital"
        ]
        self.stop_loss_percentage: float = config["bot"]["risk_management"]["stop_loss"]

        self.running: bool = True
        self.account_balance: float = config["bot"]["capital_allocation"][
            "initial_balance"
        ]  # Use initial_balance from config
    ...
    def run(self) -> None:
        ...
        while self.running:
            try:
                klines: list[list[Union[str, float, int]]] = (
                    self.exchange_client.get_historical_klines(
                        self.symbol,
                        self.timeframe,
                        limit=self.config["bot"].get(
                            "klines_limit", 500
                ...
                if self.position is not None:
                    pnl_percentage: float = (
                        latest_price - self.position["price"]
                    ) / self.position["price"]
                    if pnl_percentage < -self.stop_loss_percentage:
                        logging.warning(
                            f"Stop-loss triggered at {pnl_percentage:.2%}. Selling position."
                        )
                        self.execute_sell()
                        continue
                ...
                logging.error(
                    f"An error occurred in the trading loop: {e}", exc_info=True
                )
    ...
    def execute_sell(self) -> None:
        if self.position is not None:
            logging.info(
                f"Executing SELL order for {self.position['quantity']} {self.config['bot']['symbol']}."
            )
            # self.exchange_client.create_order(...) # Uncomment for live trading
            self.account_balance += (
                self.position["quantity"] * self.position["price"]
            )  # simplified PnL
            self.position = None
        else:
