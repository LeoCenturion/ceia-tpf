...
def backtest_har_volatility(
    ...
    """
    print(f"Fetching {symbol} {timeframe} historical data from {start_date} to {end_date}...")
    ...
    if raw_data.empty or 'close' not in raw_data.columns:
        raise ValueError("Failed to fetch historical data or 'close' column is missing.")
    ...
    if len(all_dates) < min_train_size:
        raise ValueError(f"Not enough daily RV data ({len(all_dates)} days) for initial HAR model training. Need at least {min_train_size} days.")
    ...
    if first_forecast_date_idx >= len(all_dates):
        raise ValueError("Not enough data to form initial training period for backtest.")
    ...
    for i in range(first_forecast_date_idx, len(all_dates)):
        ...
        train_window_end_date = current_forecast_date - timedelta(days=1) # Train on data *before* the forecast day
        ...
        if len(train_har_df) < min_train_size:
            print(f"Skipping {current_forecast_date}: not enough data for HAR training ({len(train_har_df)}/{min_train_size})")
            continue

        try:
            forecast_rv = har_forecast(train_har_df)
            actual_rv = har_df.loc[current_forecast_date, 'RV_daily'] # Actual RV for the day being forecast
            ...
            print(f"Date: {current_forecast_date.strftime('%Y-%m-%d')}, Actual RV: {actual_rv:.8f}, Forecast RV: {forecast_rv:.8f}")
        except Exception as e:
            print(f"Error forecasting for {current_forecast_date.strftime('%Y-%m-%d')}: {e}")
            results.append({
                'date': current_forecast_date,
...
if __name__ == '__main__':
    ...
    if not backtest_results.empty:
        ...
        rmse = np.sqrt(((backtest_results['actual_rv'] - backtest_results['forecast_rv']) ** 2).mean())
