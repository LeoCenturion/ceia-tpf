...
from src.backtesting.strategies.cpcv_strategies import RSIDivergenceCPCV
...
def run(
    ...
    rsi_window: int = 14,
    divergence_period: int = 30,
    n_groups: int = 10,
    k_test_groups: int = 2,
    embargo_pct: float = 0.01,
    experiment_name: str = "CPCV_RSIDivergence",
):
    ...
    return run_cpcv_for_strategy(
        data=data,
        t1=t1,
        strategy_class=RSIDivergenceCPCV,
        strategy_params={"rsi_window": rsi_window, "divergence_period": divergence_period},
        n_groups=n_groups,
...
def main():
    parser = argparse.ArgumentParser(description="CPCV for RSIDivergence")
    ...
    parser.add_argument("--rsi-window", type=int, default=14, help="RSI period")
    parser.add_argument(
        "--divergence-period", type=int, default=30, help="Lookback bars for divergence"
    )
    ...
    parser.add_argument("--experiment-name", default="CPCV_RSIDivergence")
    ...
    run(
        ...
        divergence_period=args.divergence_period,
