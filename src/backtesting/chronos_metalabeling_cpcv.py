...
def run_cpcv_for_metalabeling_pipeline(
    ...
    try:
        ...
        logger.info(
            f"Shape after step_3_labeling_and_weighting (labels, sw, t1): {initial_labels.shape}, {initial_sample_weights.shape}, {initial_t1.shape}"
        )
        ...
        initial_sample_weights.loc[common_initial_index]
        ...
        if X_cpcv.empty or y_cpcv.empty:
            raise ValueError(
                "Aligned initial features or labels are empty after full pipeline run. Cannot proceed with CPCV."
            )

        logger.info(
            f"Initial pipeline run for CPCV partitioning complete. X_cpcv shape: {X_cpcv.shape}, y_cpcv shape: {y_cpcv.shape}"
        )
        ...
        for fold, (train_group_idxs, test_group_idxs) in enumerate(splits):
            ...
            primary_probs = fold_pipeline.primary_model_.predict_proba(
                X_test_fold_aligned
            )[:, 1]
