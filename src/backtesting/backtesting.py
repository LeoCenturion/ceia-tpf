...
class TrialStrategy(Strategy):
    ...
    def __init__(self, broker: Broker, data: pd.DataFrame, params: dict):
    ...
    def save_artifacts(self, trial: optuna.Trial, stats: dict, bt: Backtest):
    ...
    @classmethod
    def get_optuna_params(cls, _trial: optuna.Trial) -> dict:
        ...
        with mlflow.start_run(run_name=run_name, tags=tags, nested=True):
            ...
            for key, value in stats.items():
                ...
                if not np.issubdtype(type(value), np.number):
                    ...
                    continue
                sanitized_key = sanitize_metric_name(str(key))
                mlflow.log_metric(sanitized_key, float(value))
...
def optimize_classification_strategy(  # pylint: disable=too-many-arguments
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        with mlflow.start_run(run_name=run_name, tags=tags, nested=True):
            ...
            if bt:
                ...
                if os.path.exists(plot_filename):
                    mlflow.log_artifact(plot_filename)
                # Save trades
                trades_df: pd.DataFrame = pd.DataFrame(stats["_trades"])
                if not trades_df.empty:
                    trades_df.to_csv(trades_filename, index=False)
                    mlflow.log_artifact(trades_filename)
        ...
        if hasattr(strategy_instance, "y_true") and hasattr(
            ...
            if (
                len(strategy_instance.y_true) > 0  # type: ignore
                and len(strategy_instance.y_pred) > 0  # type: ignore
            ):
                f1 = f1_score(
                    strategy_instance.y_true,  # type: ignore
                    strategy_instance.y_pred,  # type: ignore
                    zero_division="warn",
                )
                stats["F1 Score"] = f1
                mlflow.log_metric("F1 Score", float(f1))

        f1_val = stats.get("F1 Score", 0.0)
...
def run_optimizations(  # pylint: disable=too-many-arguments
    ...
    actual_start_date = cast(pd.Timestamp, data.index.min()).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    actual_end_date = cast(pd.Timestamp, data.index.max()).strftime("%Y-%m-%d %H:%M:%S")
...
def run_classification_optimizations(  # pylint: disable=too-many-arguments
    ...
    # Get the actual start and end dates from the data
    actual_start_date = cast(Timestamp, data.index.min()).strftime("%Y-%m-%d %H:%M:%S")
    actual_end_date = cast(Timestamp, data.index.max()).strftime("%Y-%m-%d %H:%M:%S")
...
def combinatorial_symmetric_cv(performance_matrix: pd.DataFrame, S: int = 16):
    ...
    if T % S != 0:
        logger.debug(
            f"Warning: Data length {T} is not divisible by S {S}. Trimming data."
        )
    ...
    for i, training_indices in enumerate(training_set_indices_combinations):
        ...
        in_sample_std: pd.Series = cast(pd.Series, in_sample_returns.std())
        in_sample_sharpe: Union[pd.Series, float] = (
            in_sample_returns.mean() / in_sample_std
        ).fillna(0)
        best_strategy_column = (
            in_sample_sharpe.idxmax()
            if isinstance(in_sample_sharpe, pd.Series)
            else in_sample_sharpe
        )

        # B. Out-of-Sample Verification
        out_of_sample_returns = testing_set
        out_of_sample_std: pd.Series = cast(pd.Series, out_of_sample_returns.std())
        out_of_sample_sharpe: pd.Series = (
