...
def plot_trades(transactions_file, symbol):
    ...
    ohlc_df = get_historical_data(
        symbol, Client.KLINE_INTERVAL_1MINUTE, str(start_date), str(end_date)
    )
    ...
    marker_styles = {
        "macd_strategy": {
            "BUY": {"marker": "^", "color": "g"},
            "SELL": {"marker": "v", "color": "r"},
        },
        "random_strategy": {
            "BUY": {"marker": "^", "color": "b"},
            "SELL": {"marker": "v", "color": "orange"},
        },
    }

    # Iterate through each unique strategy to plot its trades
    for strategy_name in trades_df["strategy"].unique():
        strategy_buy_signals = trades_df[
            (trades_df["side"] == "BUY") & (trades_df["strategy"] == strategy_name)
        ]
        strategy_sell_signals = trades_df[
            (trades_df["side"] == "SELL") & (trades_df["strategy"] == strategy_name)
        ]
        ...
        if buy_markers.notna().any():
            style = marker_styles.get(strategy_name, {}).get(
                "BUY", {"marker": "^", "color": "g"}
            )
            add_plots.append(
                mpf.make_addplot(
