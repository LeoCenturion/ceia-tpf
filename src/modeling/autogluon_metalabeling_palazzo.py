...
class PalazzoMetaLabelingPipeline(PalazzoXGBoostPipeline):
    ...
    def generate_oof_predictions(self, X, y, t1, sw, model):
        ...
        for i, (train_idx, val_idx) in enumerate(cv.split(X, y)):
            ...
            if self.config.get("use_pca", False):
                ...
                pca = PCA(n_components=self.config.get("pca_components", 0.95), random_state=42)
        ...
        return pd.DataFrame({
            'true_label': y,
            'primary_pred': oof_preds,
            'primary_prob': oof_probs
        }, index=X.index)
    ...
    def run_metalabeling_experiment(self, raw_data, primary_model, meta_model_config):
        ...
        t1_test = t1.iloc[split_idx:] # Not strictly needed for test eval but good for consistency
        ...
        X_meta_train['primary_pred'] = oof_df['primary_pred'] # Optional: Include the hard prediction
        
        print(f"Meta-Label Distribution: {meta_labels.value_counts(normalize=True).to_dict()}")
        ...
        meta_test_pred = meta_model.predict(X_meta_test) # 1 = "Primary is Correct", 0 = "Primary is Wrong"
