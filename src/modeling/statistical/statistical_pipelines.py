...
def run_statistical_optimization(
    ...
    """
    Optimize a statistical classifier with Optuna and log to MLflow.
    ...
    with mlflow.start_run(run_name=f"{model_class.__name__}_optuna") as parent_run:
        ...
        mlflow.log_metric("best_avg_cv_f1", best_value)

    print(f"\n{model_class.__name__} — best params: {best_params}")
    print(f"  best avg CV F1: {best_value:.4f}")

    # Final canonical run with best params via run_pipeline
    run_pipeline(
        pipeline=StatisticalModelPipeline(pipeline_config),
