...
class ChronosStrategy(Strategy):
    ...
    min_train_bars = refit_every * 2 # Require at least 2 refit periods of data to train
    ...
    def next(self):
        ...
        if self.periods_since_refit >= self.refit_every and len(self.data.df) >= self.min_train_bars:
...
def main():
    ...
    bt = Backtest(data, ChronosStrategy, cash=10000, commission=.002)
    ...
    if isinstance(stats, pd.Series) and not stats.empty:
        ...
        print("Backtest did not produce any stats. This might happen if no trades were executed.")
